...
def show_plot(tensor: Tensor[float_type]) -> None:
    ...
    image = light.shade(numpy_arr, plt.cm.inferno, colors.PowerNorm(0.3), 'hsv', 0, 0, 1.5)
    plt.imshow(image)
    plt.axis("off")  
    plt.savefig('./Images/juliafractal2.png')  
    plt.show()  
...
fn vectorized():
    ...
    fn worker(row: Int):
        ...
        fn compute_vector[simd_width: Int](col: Int):
            ...
            # Ensure SIMD width is consistent across all function calls
            t.data().store(
                row * width + col, julia_kernel_SIMD[simd_width](cx, cy)
            )
